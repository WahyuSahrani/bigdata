...
```
    ...
    ~\Anaconda3\lib\site-packages\pandas\core\ops.py in na_op(x, y)
       1496         try:
    -> 1497             result = expressions.evaluate(op, str_rep, x, y, **eval_kwargs)
    ...
    ~\Anaconda3\lib\site-packages\pandas\core\ops.py in na_op(x, y)
       1524                     with np.errstate(all='ignore'):
    -> 1525                         result[mask] = op(xrav, y)
       1526             else:
    ...
    <ipython-input-9-0062eb02e6ea> in <module>()
    ...
    ~\Anaconda3\lib\site-packages\pandas\core\ops.py in f(self, other, axis, level, fill_value)
       1557                 self = self.fillna(fill_value)
       1558 
    -> 1559             return self._combine_const(other, na_op, try_cast=True)
       1560 
       1561     f.__name__ = op_name
    ...
    TypeError: Could not operate 5 with block values can only concatenate str (not "int") to str
